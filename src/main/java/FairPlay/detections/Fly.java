                ...
                for (int x = -5; x <= 5; ++x) {
                    for (int y = -255; y <= 255; ++y) {
                        for (int z = -5; z <= 5; ++z) {
                            ...
                            Logger.log(e.getPlayer().getDisplayName() + " failed Fly (had 0.7 blocks per sec speed; jumped on slime blocks; there is a block in 5 radius) VL 30% | Is authed: " + e.getPlayer().hasPermission("ac.execnpccmds"));
                            return;
                        }
                    }
                }
